...
该配置的严格文件留出test accuracy/macro-F1为`0.8459/0.8452`。以validation accepted accuracy `0.90`为目标动态得到阈值约`0.712`，冻结到test后coverage `0.8767`、accepted accuracy `0.8954`、accepted macro-F1 `0.8895`。这说明置信度排序有一定作用，但不能把模型包装成全覆盖`0.90`；全覆盖指标始终是主结果。`predict-raw-cache`已经支持当前PFT checkpoint，端到端直接读取CCF5的`classify-ccf`编排仍待模型最终冻结后收尾。
...
5–9类的正式扩展采用预注册的“LB01/LB12困难对锚定 + 参考基因组组成多样性”嵌套方案，每个类别数单独训练，不做test后删类。物种清单、划分和执行命令见[5–9类分类扩展协议](docs/CLASS_COUNT_SCALING_PROTOCOL.md)。
...
核心报告和推理模块依赖 NumPy、scikit-learn、PyTorch 和 Matplotlib。CCF5读取与 Bonito embedding 提取需要在相应运行环境中安装 pyccf5 和 Bonito。第三方库按各自许可证使用，不属于本项目自研源代码。
